function status = SL2PD(Version_Name, Overwrite, Debug, P3)
% Simplified Level 2 Processor CCRS
%   status = SL2PC(Version_Name,Overwrite Debug) returns status of SL2PD
...
% P3 - optional third parameter for the LAI prior pdf
%
% Outputs
%
...
for Class = 1:Def_Base.Num_Classes
    ...
    try
        ...
        [Law]=Create_Law_Var(Def_Base,Class,[],Law,Nb_Sims,P3);
        try
            save(fullfile([Def_Base.Report_Dir '\Class_' num2str(Class)],[char(Def_Base.Name) '.mat']),'-mat','-append' )
        catch
            save(fullfile([Def_Base.Report_Dir '\Class_' num2str(Class)],[char(Def_Base.Name) '.mat']),'-mat' )
