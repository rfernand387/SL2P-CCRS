...
for isim=1:Nb_Sims % boucle sur les simulations
    ...
    elseif strcmp(Def_Base.RTM,'4sail2')
        Rs = repmat(Law.Bs(isim)*Def_Base.(['Class_' num2str(Class)]).R_Soil.Refl(51:2051,Law.I_Soil(isim)),1,4); %  Réflectance du sol dans le cas lambertien
        Car = Law.Cab(isim)/4;
        Ant = 0;
        
        %  Reflectance top of canopy
        R=PRO4SAIL2([],[],Law.N(isim),Law.Cab(isim),Car,Ant,Law.Cbp(isim),Law.Cw(isim),Law.Cdm(isim), ...
            0,0,40,2,Law.ALA(isim),0,Law.LAI(isim),Law.HsD(isim), ...
            Law.Sun_Zenith(isim),Law.View_Zenith(isim),Law.Rel_Azimuth(isim),Rs(:,1),0,0,Law.Crown_Cover(isim),2,'4SAIL2',[],Law.Age(isim));

        % D
        RsD = Rs(lambdaref-400,:)*0;
        Dest=PRO4SAIL2D(lambdaref,[],[],Law.N(isim),Law.Cab(isim),Car,Ant,Law.Cbp(isim),Law.Cw(isim),Law.Cdm(isim), ...
            0,0,40,2,Law.ALA(isim),0,Law.LAI(isim),Law.HsD(isim), ...
            Law.Sun_Zenith(isim),Law.View_Zenith(isim),Law.Rel_Azimuth(isim),Rs(:,1),0,0,Law.Crown_Cover(isim),2,'4SAIL2',[],Law.Age(isim));
        D(isim,1) = Dest(1);
        ...
        %  fAPAR (black-sky à 10h)
        A=PRO4SAIL2([],[],Law.N(isim),Law.Cab(isim),Car,Ant,Law.Cbp(isim),Law.Cw(isim),Law.Cdm(isim), ...
            0,0,40,2,Law.ALA(isim),0,Law.LAI(isim),Law.HsD(isim), ...
            Law.Sun_Zenith_FAPAR(isim),0,0,Rs(:,1),0,0,Law.Crown_Cover(isim),2,'4SAIL2',[],Law.Age(isim));
        Ecl=Sol_Irr(:,round(Law.Sun_Zenith(isim)./5)+1);
        FAPAR(isim,1)  = sum(A(1:300,5).*Ecl(1:300))./sum(Ecl(1:300)); % valeur intégrée entre 400 et 700 nm
        
        %   Albedo (black-sky pour la direction de visée du satellite)
        Albedo(isim,1) = sum(A(:,3).*Ecl)/sum(Ecl); % intégration spectrale
