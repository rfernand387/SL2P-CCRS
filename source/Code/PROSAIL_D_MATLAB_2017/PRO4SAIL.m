...
rho	=	LRT.Reflectance(1:2001);
tau	=	LRT.Transmittance(1:2001);
...
%% treat CC as a clumping index , decrease LAI due to clumping RF April 2021
laie = cc*lai;
...
%%
	...
	J1ks    = Jfunc1(ks,m,laie);
	J2ks    = Jfunc2(ks,m,laie);
	J1ko    = Jfunc1(ko,m,laie);
	J2ko    = Jfunc2(ko,m,laie);
	...
	if (alf==0)
		...
		tsstoo = tss;
		sumint = (1-tss)/(ks.*laie);
    else
		%	Outside the hotspot
		fhot=laie.*sqrt(ko.*ks);
		...
		for i=1:20
			...
			y2=-(ko+ks).*laie.*x2+fhot.*(1.-exp(-alf.*x2))/alf;
			f2=exp(y2);
...
rsot=rsost+rsodt;
  ...
  %% calcul des reflectances de la végétation;
    ...
    %% correction for cc - we now treat this as clumping
    % first convert cc to crown cover
    %cc = ( 1- exp(-sqrt(ksli*koli)*laie))/( 1- exp(-sqrt(ksli*koli)*lai))
    cc = 1;
