...
% ' @import prospect
% ' @export
function Ref  = PRO4SAIL2D(lambdaref,Spec_Sensor,Input_PROSPECT,N,CHL, ...
                     CAR,ANT,BROWN,EWT, ...
                     LMA,PROT,CBC,alpha, ...
                     TypeLidf,LIDFa,LIDFb,lai, ...
                     q,tts,tto,psi,rsoil, ...
                     fraction_brown, diss, Cv,Zeta, ...
                     SAILversion,BrownVegetation,NeedleToShootRatio)
  ...
  if (isempty(Input_PROSPECT))
    ...
    % Adjust overstory LAi for shoot clumping
  lai(1) = lai(1)/ NeedleToShootRatio;
...
%                              Input_PROSPECT.alpha(1));
    ...
    %Adjust Reflectance and Transmittance for Shoot Clumping
    %Smolander and Stenberg, 2003. A method to account for shoot scale clumping in
    %coniferous canopy reflectance models, RSE, 88,363-373,
    %https://doi.org/10.1016/j.rse.2003.06.003.
    % Where NeedleToShootRatio = 4STAR
    psh = 1- 1/NeedleToShootRatio;
    GreenVegetation.Albedo = GreenVegetation.Reflectance + GreenVegetation.Transmittance;
    GreenVegetation.Reflectance = GreenVegetation.Reflectance .* ( 1- psh) ./ ((1-psh*GreenVegetation.Albedo));
    GreenVegetation.Transmittance = GreenVegetation.Transmittance .* ( 1- psh) ./ ((1-psh*GreenVegetation.Albedo));

  % only use reference wavelength for D
    indexD = find(GreenVegetation.lambda==lambdaref);
    rho	=	GreenVegetation.Reflectance(indexD)./(GreenVegetation.Reflectance(indexD)+GreenVegetation.Transmittance(indexD));
    tau	=	GreenVegetation.Transmittance(indexD)./(GreenVegetation.Reflectance(indexD)+GreenVegetation.Transmittance(indexD))-1e-8;
    GreenVegetation.Reflectance = rho;
    GreenVegetation.Transmittance = tau;
    GreenVegetation.lambda = lambdaref;
...
%                                         Input_PROSPECT.alpha(2))
      ...
      % only use reference wavelength for D
        indexD = find(BrownVegetation.lambda==lambdaref);
        rho	=	BrownVegetation.Reflectance(indexD,2)./(BrownVegetation.Reflectance(indexD,2)+BrownVegetation.Transmittance(indexD,3));
        tau	=	BrownVegetation.Transmittance(indexD,3)./(BrownVegetation.Reflectance(indexD,2)+BrownVegetation.Transmittance(indexD,3))-1e-4;
        BrownVegetation.reflectance = rho;
        BrownVegetation.transmittance = tau;
        BrownVegetation.lambda = lambdaref;
...
% format for SL2P
Ref = [Ref.rsot Ref.rdot Ref.rsdt Ref.rddt Ref.alfast Ref.alfadt repmat(Ref.fCover_view,size(rsoil,1),1)];
  return
...
function my_list  = fourSAIL(LeafOptics, TypeLidf, LIDFa, LIDFb, lai, q, tts, tto, psi, rsoil)
  ...
  fCover_view = []; % need to put this in RF
  alfast = [];   % need to put this in RF
  alfadt = []; % need to put this in RF
  my_list = struct("lambda",LeafOptics.lambda,"rdot", rdot,"rsot" , rsot,"rddt" , rddt,"rsdt" , rsdt,  ...
                  "alfast" , alfast, "alfadt" , alfadt,'fCover_view',fCover_view);
  return
...
function my_list = fourSAIL2(leafgreen, leafbrown, ...
  ...
  elseif (lai>0)
    ...
    for (i = 1:length(litab))
      ...
      sof = sof+sofli*lidf(i);
            ...
            %fcover computation of extinction coefficient
      resVolscatt = volscatt(tts,0,psi,ttl);
      k0li = resVolscatt.chi_o;
      k0 = k0+k0li*lidf(i);
  ...
  fCover_view = Cv*(1-exp(-k0*lai/Cv));
...
function Jout = Jfunc4(m,t)
  ...
  del = m*t;
  Jout = 0*del;
  Jout(del>1e-3) = (1-exp(-del))/(m*(1+exp(-del)));
  Jout(del<=1e-3) = 0.5*t*(1.-del*del/12.);
  return
